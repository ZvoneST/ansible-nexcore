...
![Verify dev and ops user groups](nexcore-images/01_verify_dev_and_ops_user_groups.png)
...
![MediaWiki main page](nexcore-images/02_mediawiki_main_page_access.png)

*Picture 2. MediaWiki Main Page at `wiki.nexcore.local` — "MediaWiki has been installed", served by Apache on servera.*

![WordPress main page](nexcore-images/03_wordpress_main_page_access.png)

*Picture 3. WordPress public site at `www`/`nexcore.local` — NexCore landing page.*

![WordPress admin dashboard](nexcore-images/04_wordpress_admin_dashboard_access.png)
...
![DNS records for MediaWiki and mail](nexcore-images/05_dns_records_for_mediawiki_and_mail.png)

*Picture 5. dnsmasq resolving the `wiki` A record to 192.168.50.85 and the domain MX record to `servera`.*

![HTTP status for MediaWiki and WordPress](nexcore-images/06_http_status_for_mediawiki_and_wordpress.png)
...
![NFS export configuration](nexcore-images/07_nfs_export_configuration_check.png)

*Picture 7. `exportfs -v` on serverb — `/mnt/shared` exported to the LAN with `sec=sys,rw`.*

![Shared directory access and permissions](nexcore-images/08_shared_directory_access_and_file_permissions.png)
...
![Internal email delivery test](nexcore-images/09_internal_email_delivery_test.png)

*Picture 9. Mail from `ops01` → `dev02@nexcore.local` delivered into `dev02`'s Maildir, shown via `cat`.*

![Postfix mail delivery log](nexcore-images/10_postfix_mail_delivery_log_check.png)
...
![Remote logging from serverb](nexcore-images/11_remote_logging_from_serverb_check.png)
...
![firewalld running and enabled](nexcore-images/12_firewalld_running_and_enabled_check.png)

*Picture 12. firewalld `--state` = running, `is-active` = active, `is-enabled` = enabled on servera and serverb.*

![SELinux enforcing mode](nexcore-images/13_selinux_enforcing_mode_check.png)
...
![Database backup script and cron](nexcore-images/14_database_backup_script_and_cron_check.png)
...
![Dovecot status and IMAP port](nexcore-images/15_dovecot_service_status_and_imap_port_check.png)
...
![Send test mail and verify Maildir delivery](nexcore-images/16_send_test_mail_and_verify_maildir_delivery.png)

*Picture 16. Test mail from `dev01` → `mkt01` delivered into `mkt01`'s `Maildir/new/`.*

![mutt mailbox message list](nexcore-images/17_mutt_mailbox_test_messages_list.png)

*Picture 17. `mutt` connected over IMAP listing the delivered test messages in the mailbox.*

![mutt opened IMAP message](nexcore-images/18_mutt_opened_imap_test_message.png)
